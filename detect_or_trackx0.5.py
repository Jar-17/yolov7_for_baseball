...
def detect(save_img=False):
    ...
    t0 = time.time()
    ###################################
    global frame_num , swing_time ,max_dis
        ...
        for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(tracked_dets)>0:
                        ...
                        if opt.show_track:
                            ...
                            for t, track in enumerate(tracks):
                                ...
                                #         dis[i] = opt.proportion*(((int(track.centroidarr[i][0])-int(track.centroidarr[i+1][0]))**2+(int(track.centroidarr[i][1])-int(track.centroidarr[i+1][1]))**2)**(1/2))*opt.framerate
                                ...
                                #     slope_part = (attack_angle[10][1]-attack_angle[0][1])/(attack_angle[10][0]-attack_angle[0][0]) 
                                ...
                                #     if i> 10:
                                #         swing_time+=1
                                # swing_time = swing_time/opt.framerate
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    [cv2.putText(im0,'frame_num: %d' % frame_num,(20, 190) , cv2.FONT_HERSHEY_SIMPLEX, 0.5, shadow_color , 2)]
                    [cv2.putText(im0,'barrel speed: %.2f' % max_dis,(20, 100) , cv2.FONT_HERSHEY_SIMPLEX, 0.5, shadow_color , 2)]
                    [cv2.putText(im0,'swing time: %.4f' % total_time,(20, 130) , cv2.FONT_HERSHEY_SIMPLEX, 0.5, shadow_color , 2)]
                    #在第10個frame
                    #if i == len(pred)-10:
                    #    attack_angle = sorted(track.centroidarr , reverse = False , key=lambda x:x[1] )
                    ...
                    if i > len(pred)-10:
                        ...
                        [cv2.putText(im0,'swing angle: %.2f' % slope,(20, 160) , cv2.FONT_HERSHEY_SIMPLEX, 0.5, shadow_color , 2)]
                    frames = vid_writer.get_frames()
                    for _ in range(2):  # Repeat the entire video sequence twice
                        for im in frames:
                            vid_writer.write(im)
